...
Com.prototype.to_page = function(index) {
    ...
    CurSite.postDigest({cmd:"AH01"}, body, function(err, back_body)
    {
       if(!back_body || !back_body.data) {
           return;
       }
...
Com.prototype.get_table = function(data) {
    ...
    html += '<thead><tr><td>用户名</td><td>密码</td><td>创建时间</td><td>操作<td></tr></thead>';
    html += '<tbody>';
    for(var i = 0; i < data.length; i++) {
        html += '<tr>'
        + '<td>' + data[i].username + '</td>'
        + '<td>' + data[i].password + '</td>'
        + '<td>' + CurSite.getDateStr(data[i].reg_time*1000) + '</td>'
        + '<td>'
